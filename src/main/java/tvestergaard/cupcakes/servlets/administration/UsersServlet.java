...
import tvestergaard.cupcakes.AdministrationHelper;
...
public class UsersServlet extends HttpServlet
{
    ...
    {
        AdministrationHelper administrationHelper = new AdministrationHelper(request, response);

        if (!administrationHelper.isAdmin()) {
            administrationHelper.redirect(REFERER);
            return;
        }
    ...
    private void handleNew(HttpServletRequest request, HttpServletResponse response, Notifications notifications) throws ServletException, IOException
    {
        try {
    ...
    private void handleUpdate(HttpServletRequest request, HttpServletResponse response, Notifications notifications) throws ServletException, IOException
    {
        int id = 0;
    ...
    private void handleDelete(HttpServletRequest request, HttpServletResponse response, Notifications notifications) throws ServletException, IOException
    {
        if (request.getParameter("id") == null) {
            notifications.warning(Language.MISSING_ID_PARAMETER);
            response.sendRedirect(request.getHeader("referer"));
